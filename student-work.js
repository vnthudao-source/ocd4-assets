...
/* =========================================================
   V18.5 - REWARD CORE CANONICAL ASSETS
   Tài sản cuối cùng chỉ đọc từ StudentRewardSystem dùng chung.
========================================================= */

let rewardCoreReadyPromise = null;
const canonicalAssetMap = new Map();
const canonicalAssetPromises = new Map();
...
function waitRewardCore(){
    if(isRewardCoreReady()){
        return Promise.resolve(getRewardCore());
    }
    ...
    rewardCoreReadyPromise = new Promise(function(resolve,reject){
        let tries = 0;
        const timer = setInterval(function(){
            ...
            if(tries >= 200){
                ...
                reject(new Error("StudentRewardSystem Core chưa sẵn sàng."));
            }
        },100);
    });
...
function normalizeCanonicalRewardProfile(RS,rawProfile,studentCode){
    const raw =
    ...
    if(root.gems && typeof root.gems === "object"){
        gems = root.gems;
    }else if(root.balance && typeof root.balance === "object"){
        gems = root.balance;
    }else if(
        root.rewardData &&
...
function getValidStudentWorksForCore(studentCode){
    const target = normalize(studentCode);
    ...
    return works.filter(function(item){
        const submissionId = workSubmissionId(item);

        if(
            submissionId &&
            deletedSubmissionIds.has(submissionId)
        ){
            return false;
        }
        ...
        return normalize(
            item.__code ||
            column(item,["Mã học viên"])
        ) === target;
    });
}

function buildStudentSubmissionCSVForCore(studentCode){
    return objectRowsToCSV(
        getValidStudentWorksForCore(studentCode)
    );
}

async function loadCanonicalStudentAssets(studentCode,force){
    const code = clean(studentCode);
    ...
    if(!force && canonicalAssetMap.has(key)){
        return canonicalAssetMap.get(key);
    }

    if(!force && canonicalAssetPromises.has(key)){
        return canonicalAssetPromises.get(key);
    }

    const promise = (async function(){
        const RS = await waitRewardCore();

        await RS.loadSharedRewardData(Boolean(force));
        ...
        canonicalAssetMap.set(key,canonical);

        return canonical;
    })();

    canonicalAssetPromises.set(key,promise);

    try{
        return await promise;
    }finally{
        canonicalAssetPromises.delete(key);
    }
}

async function loadCanonicalAssetsForWorks(force){
    const codes = [
        ...new Set(
            works
            .map(function(item){
    ...
    for(let start=0; start<codes.length; start+=BATCH_SIZE){
        ...
        await Promise.all(
            batch.map(function(code){
                return loadCanonicalStudentAssets(code,force)
                .catch(function(error){
                    console.warn(
                        "Reward Core không tải được hồ sơ:",
                        code,
                        error
                    );
                    return null;
...
function canonicalItemQuantity(item){
    if(typeof item === "string"){
        return 1;
    }
...
function getCanonicalAssetRecord(code){
    return canonicalAssetMap.get(normalize(code)) || null;
}
...
function buildOwnershipMap(){
    /*
       V18.5:
       Không replay PhieuDoi/QuaTangGVCN tại trang Admin.
       canonicalAssetMap đã được StudentRewardSystem tính.
    */
}

function getStudentBeasts(code,name){
    const canonical = getCanonicalAssetRecord(code);
    ...
    canonical.ownedItems.forEach(function(owned){
        if(canonicalItemQuantity(owned) <= 0){
            return;
        }

        const ownedName = canonicalItemName(owned);
        const ownedKey = normalize(ownedName);

        if(!ownedKey){
            return;
        }

        let beast = beastCatalog.find(function(item){
            return(
                ownedKey === item.key ||
...
function studentOwns(code,name,beastKey){
    const target = normalize(beastKey);

    if(!target){
        return false;
    }

    return getStudentBeasts(code,name)
    .some(function(beast){
        return(
            beast.key === target ||
            beast.key.includes(target) ||
            target.includes(beast.key)
        );
    });
}
    ...
    for(
        ...
        try{
            ...
            if(
                ...
                /*
                   QuaTangGVCN chỉ xác nhận giao dịch.
                   Quyền sở hữu cuối cùng phải được Core replay lại.
                */
...
async function handleDelete(button){
    ...
    try{
        ...
        /*
           works hiện đã loại bài vừa xóa.
           Force refresh để bài XoaBai không còn ảnh hưởng
           tới điểm thưởng/tài sản trong Reward Core.
        */
        canonicalAssetMap.delete(
            normalize(
                card.dataset.code
            )
        );
        ...
        .catch(function(error){
            console.warn(
                "Không refresh được Reward Core sau XoaBai:",
                error
            );
        });
...
async function loadData(){
    ...
    try{
        ...
        /*
           Phải chuẩn bị __code/__submissionId trước khi
           tạo CSV bài nộp đã lọc XoaBai cho Reward Core.
        */
        prepareBaseWorkCache();
...
/* =========================================================
   V18.5 - NHẬN SỰ KIỆN THAY ĐỔI PROFILE TỪ REWARD CORE
========================================================= */
...
window.addEventListener(
    ...
    function(event){
        ...
        const exists =
        works.some(function(item){
            return normalize(item.__code) === normalize(code);
        });
        ...
        if(canonicalRefreshTimer){
            clearTimeout(canonicalRefreshTimer);
        }
